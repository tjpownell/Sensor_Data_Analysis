...
Engage_Conditions = Engage_Conditions.to_numpy()
Input_Validation = Input_Validation.to_numpy()
Disengage_Conditions = Disengage_Conditions.to_numpy()
...
# Initialize a separate list for graphing locked locations on the plot
...
Disengage_Conditions = Disengage_Conditions[np.logical_not(np.isnan(Disengage_Conditions))]
Engage_Conditions = Engage_Conditions[np.logical_not(np.isnan(Engage_Conditions))]
Input_Validation = Input_Validation[np.logical_not(np.isnan(Input_Validation))]
...
Disengage_Conditions = Disengage_Conditions.tolist()
Engage_Conditions = Engage_Conditions.tolist()
...
for i in range(len(X_a_coord)):
    ...
    j = i//2
    print(Input_Validation[j])

    if Disengage_Conditions[j] - Input_Validation[j] - Engage_Conditions[j] <= -2:
        print('   ssssssssssssssssssssss')
        LX_a_coord.append(X_a_coord[i])
        LX_b_coord.append(X_b_coord[i])
        LY_a_coord.append(Y_a_coord[i])
        LY_b_coord.append(Y_b_coord[i])
...
plot.plot(X_a_coord, Y_a_coord, "-b", label="Left Wheel Speed")
plot.plot(X_b_coord, Y_b_coord, "-r", label="Right Wheel Speed")

plot.plot(LX_a_coord, LY_a_coord, "-g", label="Left Wheel Speed")
plot.plot(LX_b_coord, LY_b_coord, "-g", label="Right Wheel Speed")
